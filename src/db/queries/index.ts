import { eq, and, or, isNull, isNotNull, lt, lte, gte, ne, like, desc, sql, count } from "drizzle-orm";
...
export async function getTasksScheduledForDate(db: Db, date: string): Promise<DbTask[]> {
  return db
    .select()
    .from(schema.tasks)
    .where(
