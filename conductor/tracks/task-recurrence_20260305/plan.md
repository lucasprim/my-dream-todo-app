...
- [ ] Task 2.2: Integrate `RecurrencePicker` into `TaskEditModal` — add recurrence field alongside title and due date
- [ ] Task 2.3: Update `updateTaskAction` and `createTaskAction` to accept and persist recurrence from the UI
...
- [ ] Task 3.1: Update `completeTask()` in `task-actions-impl.ts` to distinguish between fixed-schedule (`every`) and after-completion (`every!`) modes — fixed uses due date as base, after-completion uses today's date
- [ ] Task 3.2: Handle edge cases: completing overdue recurring tasks (fixed schedule should skip to next future occurrence, not generate past dates), tasks with no due date but with recurrence
